...
from app.extensions import get_tenant_db # ✨ MULTI-TENANT İÇİN EKLENDİ

def create_cek_form(yon='alinan', islem=None, view_only=False):
    """
    yon: 'alinan' (Müşteri Çeki) veya 'verilen' (Kendi Çekimiz)
    """
    tenant_db = get_tenant_db() # ✨ Veritabanı bağlantısı alındı
    ...
    if view_only: baslik_on_ek = "Detayı" 
    ...
    # ✨ DÜZELTME: CariHesap.query yerine tenant_db kullanıldı
    ...
    if yon == 'alinan':
        ...
        # ✨ DÜZELTME: BankaHesap.query yerine tenant_db kullanıldı
    ...
    if is_edit and islem:
        ...
        cari_id.value = str(islem.cari_id) if islem.cari_id else None # ✨ TİP DÜZELTMESİ (UUID'den String'e)
        aciklama.value = islem.aciklama
        
        if yon == 'alinan':
            if hasattr(islem, 'resim_on_path') and islem.resim_on_path: cek_resmi.value = islem.resim_on_path
            ...
            if islem.banka_adi:
                for b in bankalar:
                    if b.banka_adi == islem.banka_adi and b.hesap_no == islem.hesap_no:
                        bizim_banka.value = str(b.id) # ✨ TİP DÜZELTMESİ
                        break

    # ==========================================
    # 3.VIEW MODE AYARI (Render'dan ÖNCE)
    # ==========================================
...
def create_cek_islem_form(cek):
    ...
    """
    tenant_db = get_tenant_db() # ✨ Multi-tenant bağlantısı
    ...
    # --- Veri Hazırlığı ---
    # ✨ DÜZELTME: Kasa.query vb. yerine tenant_db kullanıldı
