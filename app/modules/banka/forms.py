...
def create_banka_form(banka=None):
    ...
    # ✨ UUID UYUMU: int yerine string URL formatı
    ...
    tenant_db = get_tenant_db() # ✨ YENİ BAĞLANTI
    ...
    banka_adi = FormField(
        ...
        _('Banka Adı'), 
        required=True, 
        options=banka_listesi,
        value=banka.banka_adi if banka else '', 
        select2_config={'placeholder': 'Bankayı Seçiniz...', 'search': True} 
    )    
    ...
    hesap_turu = FormField('hesap_turu', FieldType.SELECT, _('Hesap Türü'), 
                           ...
                           value=mevcut_hesap_turu)
    
    sube_id = FormField('sube_id', FieldType.SELECT, _('Bağlı Şube'), options=sube_opts, required=True, value=str(banka.sube_id) if banka and banka.sube_id else '', select2_config={'placeholder': 'Şube Seçiniz'})
    ...
    muhasebe_hesap = FormField('muhasebe_hesap_id', FieldType.SELECT, _('Muhasebe Kodu'), options=muhasebe_opts, value=str(banka.muhasebe_hesap_id) if banka and banka.muhasebe_hesap_id else '', select2_config={'placeholder': 'Hesap Seç', 'allowClear': True})
